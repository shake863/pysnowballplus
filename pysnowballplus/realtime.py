...
def minute(symbol):
    dict_data = utls.fetch(api_ref.minute.format(symbol))
    return sw.minute_todf(dict_data)


def trade(symbol, count=20):
    dict_data = utls.fetch(api_ref.trade.format(symbol, count))
    return sw.trade_todf(dict_data)
